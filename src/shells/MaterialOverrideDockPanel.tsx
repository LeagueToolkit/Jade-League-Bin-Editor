...
import './MaterialOverrideDockPanel.css';
...
interface MaterialOverrideDockPanelProps {
...
export default function MaterialOverrideDockPanel({
  type,
  defaultPath,
  onSubmit,
  onSubmitWithLibrary,
  ...
  detectedTextures,
  binFilePath,
}: MaterialOverrideDockPanelProps) {
  ...
  // any `.mop-lib-dropdown-menu` element for containment.
  useEffect(() => {
    if (!dropdownOpen) return;
    const onDocClick = (e: MouseEvent) => {
      const target = e.target as Node;
      if (dropdownRef.current?.contains(target)) return;
      const menu = document.querySelector('.mop-lib-dropdown-menu');
  ...
  const handleTextureEnter = (tex: string, e: React.MouseEvent) => {
    ...
    // Viewport-aware placement: prefer right-of-pill, flip to left when
    // there's no room (right-side dock case), then clamp top so the
    // preview never falls below the viewport bottom.
    const PREVIEW_W = 280;
    const PREVIEW_H = 280;
    const M = 8;
    const fitsRight = rect.right + M + PREVIEW_W <= window.innerWidth;
    const left = fitsRight
      ? rect.right + M
      : Math.max(M, rect.left - M - PREVIEW_W);
    const top = Math.max(
      M,
      Math.min(window.innerHeight - PREVIEW_H - M, rect.top),
    );
  ...
  // (Title is provided by the dock pane header in this variant.)
  const pathLabel = type === 'texture' ? 'Texture Path' : 'Material name';
  const hasSuggestions = suggestions && suggestions.length > 0;

  // Dockable variant — no modal overlay, no close button. The dock
  // pane provides its own header / drag handle / × button.
  return (
    <div className="mop-dialog mop-dialog-docked" onKeyDown={handleKeyDown}>
        <div className="mop-content">
          {(hasSuggestions || (detectedTextures && detectedTextures.length > 0)) && (
            <div className="mop-field mop-sources-card">
              {hasSuggestions && (
                <div className="mop-sources-section">
                  <label className="mop-label">Submeshes from SKN</label>
                  <div className="mop-pill-list">
                    {suggestions!.map((s, i) => (
                      <button
                        key={i}
                        className={`mop-suggestion ${submesh === s.material ? 'active' : ''}`}
                        onClick={() => applySuggestion(s)}
                        title={s.texture || 'No texture match'}
                      >
                        <span className="mop-suggestion-name">{s.material}</span>
                        {s.texture ? (
                          <span className="mop-suggestion-match">matched</span>
                        ) : (
                          <span className="mop-suggestion-nomatch">no match</span>
                        )}
                      </button>
                    ))}
                  </div>
              ...
              {hasSuggestions && detectedTextures && detectedTextures.length > 0 && (
                <div className="mop-sources-divider" />
              )}

              {detectedTextures && detectedTextures.length > 0 && (
                <div className="mop-sources-section">
                  <label className="mop-label">Textures from folder</label>
                  <div className="mop-pill-list">
          ...
          <div className="mop-field">
            <label className="mop-label">{pathLabel}</label>
            <input
              type="text"
              className={`mop-input mop-input-path ${libraryEnabled && selectedLibPath ? 'mop-input-readonly' : ''}`}
              value={path}
          ...
          <div className="mop-field">
            <label className="mop-label">Submesh name</label>
            <input
              type="text"
              className="mop-input"
              value={submesh}
          ...
          {type === 'material' && libraryEnabled && (
            <div className="mop-field">
              <label className="mop-label">Texture Path</label>
              <input
                type="text"
                className="mop-input mop-input-path"
                value={texture}
          ...
          {type === 'material' && onSubmitWithLibrary && (
            <div className="mop-library-section">
              <label className="mop-library-toggle">
                <input
                  type="checkbox"
              ...
              {libraryEnabled && (
                <div className="mop-library-body">
                  {libraryError && (
                    <div className="mop-library-error">{libraryError}</div>
                  )}

                  {!libraryIndex && !libraryError && (
                    <div className="mop-library-empty">Loading library…</div>
                  )}

                  {libraryIndex && selectorItems.length === 0 && (
                    <div className="mop-library-empty">
                  ...
                  {selectorItems.length > 0 && (
                    <div className="mop-lib-dropdown" ref={dropdownRef}>
                      <button
                        type="button"
                        className={`mop-lib-dropdown-trigger ${dropdownOpen ? 'open' : ''}`}
                        onClick={() => dropdownOpen ? setDropdownOpen(false) : openDropdown()}
                      >
                        <span className="mop-lib-dropdown-value">
                          {selectedEntry ? (
                            <>
                              <span className="mop-lib-dropdown-name">{selectedEntry.name}</span>
                              <span className="mop-lib-dropdown-version">v{selectedEntry.version}</span>
                            </>
                          ) : (
                            <span className="mop-lib-dropdown-placeholder">Select a material…</span>
                          )}
                        </span>
                        <svg className="mop-lib-dropdown-chevron" width="12" height="8" viewBox="0 0 12 8" fill="none">
                          <path d="M1 1.5L6 6.5L11 1.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      </button>
                      {dropdownOpen && menuStyle && createPortal(
                        <div className="mop-lib-dropdown-menu" style={menuStyle}>
                          {selectorItems.map((entry) => {
                            ...
                            return (
                              <button
                                key={entry.path}
                                type="button"
                                className={`mop-lib-dropdown-item ${active ? 'active' : ''}`}
                                onClick={() => {
                                  setSelectedLibPath(entry.path);
                                  setDropdownOpen(false);
                                }}
                              >
                                <span className="mop-lib-dropdown-name">{entry.name}</span>
                                <span className="mop-lib-dropdown-version">v{entry.version}</span>
                              </button>
                            );
                          })}
                        </div>,
        ...
        {error && (
          <div className="mop-error">{error}</div>
        )}

        <div className="mop-actions">
          {/* Cancel is omitted — the dock pane's × handles dismissal. */}
          <button className="mop-btn mop-btn-accept" onClick={handleSubmit} disabled={busy}>
            {busy ? 'Working…' : 'Add entry'}
          </button>
        </div>

      {/* Texture hover preview — portalled to <body> so it escapes any
          `backdrop-filter` ancestor (modern UI's frosted-glass dialog
          creates a containing block for position:fixed children, which
          would otherwise trap the preview inside the dock pane). */}
      {hoverTexture && hoverPos && createPortal((() => {
        const cached = previewCache[hoverTexture];
        return (
          <div
            className="mop-texture-preview"
            style={{ top: hoverPos.top, left: hoverPos.left }}
          >
            {cached === undefined ? (
              <div className="mop-texture-preview-status">Loading…</div>
            ) : cached === null ? (
              <div className="mop-texture-preview-status">No preview available</div>
            ) : (
              <img src={cached} alt="" />
            )}
            <div className="mop-texture-preview-path">
